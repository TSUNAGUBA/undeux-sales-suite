...
- **写真帳**（`/mart/photobook`） — 商品を投入企画書レイアウトのスペックシート（品番・品名・PB・素材・上代・画像・サイズ／カラー・投入日・受注日・先付 等）で一覧。商品マスタに無い項目は空白。1〜4アイテムを選択して Excel（.xlsx）出力可。フィルターは業態・部門／品名（部分一致）／商品記号（部分一致）／品番3桁
