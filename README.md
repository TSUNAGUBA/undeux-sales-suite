...
- **アイテム分析**（サプライヤー） — 商品別分析／商品詳細分析／写真帳／週間モニタリング／日次分析：特定品番／ブランド・シリーズ分析／商品導入管理
- **データ管理** — 予算管理／商品マスタ／週次取込（商品マスタ・週次取込はサプライヤーのみ）
- **探索・予測分析** — クロス集計／ランキング分析／散布図・回帰分析／重回帰シミュレーター

> ホーム（`/`）はサプライヤー向けに、全社サマリーのカード型KPIと「本日のインサイト & おすすめ」
...
- **商品詳細分析**（`/mart/item-detail`） — SKU（品番3桁×単品4桁）単位の「売数／在庫数／在日」を週別（週を列展開）／当週（売上参照ファイル準拠）で分析。在日の色分け（〜30赤／31-60青／61〜既定）・値下げ（売価変更）週のセル強調・消化率（累計／プロパー／値下げ）の分解・表示範囲サマリー・Excel 貼り付け用コピーを提供。フィルターは業態・部門／品名（部分一致）／商品記号（部分一致）／品番3桁／期間。値下げ検出・消化率分解・表示切替はフロントの表示射影（SoT は集計素材）
- **写真帳**（`/mart/photobook`） — 商品マスタを画像主体で閲覧するギャラリー。商品/SKU 情報（品番・記号・品名・価格・色数・サイズ数・SKU数・売上数量・在庫）をカード表示。フィルターは業態・部門／品名（部分一致）／商品記号（部分一致）／品番3桁。カード押下で商品の詳細分析へ
...
- **ランキング分析**（`/mart/ranking`） — 単軸ランキングに「複合スコアリング」「期間比較・順位変動」「ABC/パレート分析」の3レンズを統合。商品記号別では変動列の右に「残在庫数」「残在庫金額（原価）」を併記
