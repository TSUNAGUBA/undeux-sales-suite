...
- **副資材チェック**（`/subsidiary-check`・サプライヤーのみ） — 指示書（工程表）画像とタグ（下げ札・品質表示）画像をアップロードし、AI が**レイアウト／順番／内容**の3要素を比較チェック（判定: 合格／要確認／要修正）。商品マスタの付属情報が登録済みならチェックの元ネタとして突合。ページ内3タブ（チェック履歴／新規チェック／ルールブック、`?tab=` で直リンク可）。チェック記録・画像は PostgreSQL に永続化し、再実行は失敗時・処理中断時のみ可能（確定済み判定は保護）。ルールブックはしまむら副資材規定（アイコン優先順位・禁止用語・表示順序・寸法）のカタログ。**要 `ANTHROPIC_API_KEY`（未設定時は 503「AI未設定」）**（設計: `docs/design.md` §13）
