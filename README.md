...
- **業務チャット**（`/ai/business-chat`） — 「システム部（Web-EDI・受発注）」「商品管理部（品質管理・品質規格）」「物流部（直流・納品・物流）」のいずれかを先に選び、蓄積ナレッジ（RAG）を根拠に AI が業務質問へ回答（出典明示・根拠がなければ相談窓口を案内）。SSE ストリーミング応答。**要 `ANTHROPIC_API_KEY`（未設定時は「AI未設定」表示）**
- **商談チャット**（`/ai/negotiation-chat`） — 上部の業態 chips → 配下部門 chips で相手を選び、AI が当該業態×部門のしまむらバイヤーとして商談シミュレーション。蓄積ナレッジに加え、分析マート（スタースキーマ）から業態次元で決定的に集計した実績（週次実績・売上上位商品・在庫スナップショット）を根拠に応対する。**要 `ANTHROPIC_API_KEY`**
