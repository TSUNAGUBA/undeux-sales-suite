...
`/`（ホーム）で目的カテゴリを選び、配下ページをタブで切り替える。

- **業績モニタリング** — 全社サマリー／売上分析／在庫・発注分析
- **商品分析** — 商品別分析／商品導入管理
- **探索・予測分析** — クロス集計／ランキング分析／散布図・回帰分析／重回帰シミュレーター
- **データ管理** — 商品マスタ／週次取込

- **全社サマリー**（`/mart`） — 主要KPI（売上金額・数量・粗利・粗利率・在庫・消化率・商品/SKU数）と週次トレンド、mart の再構築
- **売上分析**（`/mart/sales`） — 週次売上推移グラフ（売上数量/売上金額=折れ線、店頭在庫=棒、気温=折れ線）・週次明細・集計軸別の売上構成
- **商品別分析**（`/mart/products`） — 画像カードの商品一覧。カード押下で商品の詳細分析（画像・基本情報・サマリー・SKU情報・週次売上推移グラフ・クロス集計）へ
- **在庫・発注分析**（`/mart/inventory`） — 在庫数・発注数・先付数・消化率・在庫日数（最新週スナップショット基準）
...
フィルタは「フィルタ → 集計単位 → 表示集計値」の導線で統一。業態はタグ（複数選択）、共通フィルタに
**棚割1** と **平均在庫日数（在日）**（◎30日以内／〇31〜60日／△61日以上）を持つ。
