...
- **Mart分析（スタースキーマ）** — 分析用ディメンショナルモデル（`mart` スキーマ）から集計。`sales_weekly` とは別系統で、他小売・他メーカーにも展開可能な汎用構造。週次フローのKPI・トレンド・集計軸別ランキングを表示し、`sales_weekly` ＋ 商品マスタから派生データを再構築（設計: `docs/star-schema-design.md`）

共通フィルタに **棚割1** と **平均在庫日数**（◎30日以内／〇31〜60日／△61日以上）を追加。気温は標準的な
日本の気候（気象庁平年値ベースのサンプルデータ）を用い、標準＝東京・寒冷＝札幌・温暖＝那覇で参照する。
