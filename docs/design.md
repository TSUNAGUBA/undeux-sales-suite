...
- **AI 呼出は同時実行 1 件に直列化**（SemaphoreSlim）し、順番待ちは 420 秒（7分）・AI 呼出自体は 120 秒で
  ...
  **この収支は受付枠の予約を画像バッファ確保より前に行うことを前提とする**（`AiCheckSlotLease`。
  拒否される要求が 20MB を確保してから弾かれると前提が崩れる）。
  **収支外の経路（受容した既知のリスク）:** 次の2つは受付枠・セマフォの管理外であり、
  上記の約310MB には含まれていない。同時実行が重なるとヘッドルーム（約74MB）を侵食しうる。
  - **画像配信**（`GET /api/subsidiary-check/{id}/images/{imageId}`）: 詳細画面は最大13枚を並列取得し、
    サーバー側は1枚ずつ byte[] を実体化して返す。1チェック分は合計20MB 以内だが、**同時閲覧者数に比例**する。
  ...
  **定量的な境界:** 画像配信は1閲覧あたり byte[] 20MB ＋ Npgsql の一時割当 同量程度 ≒ **約40MB** を要するため、
  **詳細画面を同時に2名が開くとヘッドルーム約74MB を超えうる**（40MB × 2 = 80MB）。
  現状の利用規模（日次数件〜10件・少人数）ではこの同時性は稀として受容するが、
  **成立条件は「api コンテナのメモリ監視が機能していること」**である（手順は `infra/deploy-guide.md`
  ステップ8-2 の4項目目。ホスト全体のメモリ指標ではコンテナ OOM を検知できないため、
  ...
  ASP.NET Core のベースライン（約100〜150MB）と合わせてハードリミットに到達する。
...
- **画像ストレージの増加:** チェック1件あたり最大 合計20MB の画像を PostgreSQL（bytea）に保存し、
  ...
  運用開始時に **RDS のストレージ使用率アラーム（閾値・通知先）を設定すること**を前提とする
  （手順は `infra/deploy-guide.md`）。将来の恒久対応として「判定・findings は恒久保存、画像バイナリのみ
