...
このロジックは `WeekCalendar`（Core）と `v_sales_daily` ビュー（DB）の双方で同一に実装する。
...
| `v_sales_daily` | 日次売上ビュー。週次ファクトの日次7列を縦展開 |
...
| POST | `/api/imports` | 週次CSV取込（multipart） |
...
- **パフォーマンス:** 約160万行に対する集計クエリは `import_date` 等のインデックスで
  範囲を絞る。重い集計はフロントエンドのスケルトン表示で体感を補償する。
...
- バックエンド: ユニット・統合テスト 68 件がパス。初期DBダンプ（約160万行）の投入を確認。
