...
- **非同期実行（バックグラウンド＋ポーリング）:** AI チェックは POST リクエスト内で実行しない。
  ...
  **根拠:** 公開経路は複数プロジェクト相乗りの共有 `nginx-proxy`（読み取りタイムアウト約60秒）で、
  AI 呼出は順番待ちを含め最大約9分（540秒）かかりうる。同期実行では利用者に 504 が返る一方サーバー側は completed で
...
- **記録保護と手動回復:** チェック記録は記録系データ（原則2）。AI 呼出失敗・応答解析失敗・キャンセルは
  failed 状態＋エラー内容で記録に残し、主要フロー（登録済み記録の参照）を止めない（原則4）。
  AI 未設定（キーなし）のみ永続化前に 503 で弾く（無駄な記録を作らない）。
  再実行（rerun）は failed に加え、プロセス停止等で孤児化した processing（最後に実行を開始してから
  滞留上限を超過したもの）にも許可する（手動回復パス）。completed の上書きは 400 で拒否し、
  DB 層でも `status <> 'completed'` ガードで二重に保護する。rerun の入口では DB 上で
  `processing` への原子的な状態遷移（条件付き UPDATE の影響行数が 1 のときのみ実行）を行い、
  連打・並行実行による AI 呼出の多重発生を防ぐ。
...
- **AI 呼出は同時実行 1 件に直列化**（SemaphoreSlim）し、順番待ちは 420 秒（7分）・AI 呼出自体は 120 秒で
  ...
  **収支外の経路:** 次の2つは AI 実行の受付枠・セマフォの管理外であり、上記の約260MiB には含まれない。
  - **DB 読取の一時コピー**: Npgsql は 8KB を超える行にオーバーサイズバッファを確保し、接続がプールへ
    ...
    （rerun の画像読取・下記の画像配信の両方に効く）。
  - **画像配信**（`GET /api/subsidiary-check/{id}/images/{imageId}`）: 詳細画面は最大13枚を取得し、
    サーバー側は1枚ずつ byte[] を実体化して返す。**同時実行数を専用のセマフォで 4 に有界化する**
    （`SubsidiaryCheckService.MaxConcurrentImageDownloads`）。1枠のピークは区間で異なる:
    **DB 読取中**が byte[]（最大5MiB）＋上記 Npgsql の一時割当（同量程度）＝**約10MiB**、
    **応答書き出し中**は接続がプールへ返り一時割当が消えるため byte[] のみ＝**約5MiB**
    （Kestrel は応答を 64KiB 単位でバックプレッシャー付きに書くため、本文のコピーは積み上がらない）。
    全枠が同時に読取中という最悪ケースの 4 × 10MiB ＝ **約40MiB がこの経路の上限**。
    **同時閲覧者が何人いてもこの値を超えない**（閲覧者数に比例しない）ことが要点で、これは
    **枠を応答本文の書き出し完了まで保持する**ことで成立する。`IActionResult` を返して MVC に
    本文を書かせると書き出しはアクション復帰後＝枠の外になり、その区間の byte[] が野放しになるため、
    コントローラは本文を自分で書き切ってから枠を返している。
    書き出しに要する時間は「本文を誰がどれだけ速く引き取るか」に依存する。共有 `nginx-proxy` が
    応答をバッファする設定であれば、書き出しはコンテナ間の速度で完了し利用者の回線速度に引きずられない。
    ただしプロキシの設定は本リポジトリの管理外であり、**有界化の保証をその設定に依存させない**
    ために枠は書き出しまで保持する（保持しすぎる方向の誤差は安全側）。
    順番待ちが 30 秒を超えた場合は 429（UNDX-REQ-009）で返し、利用者は再読込で復帰する。
    クライアント側も同時取得を 3 に制限し（`SUBSIDIARY_GALLERY_FETCH_CONCURRENCY`）、
    1名が枠を占有して他の閲覧者を 429 にしないようにする。
  ...
  **定量的な境界:** AI 実行の約260MiB に画像配信の上限 約40MiB を加えても約300MiB で、
  ...
  コンテナ粒度の値を発行したうえでアラームを張る）。
  恒久対応は接続文字列の読取バッファ指定または `CommandBehavior.SequentialAccess` 化を検討する。
  なお 5 秒間隔のポーリング（`GET /api/subsidiary-check/{id}`）は画像メタのみを取得し
...
- system プロンプトに反プロンプトインジェクション文を明記する（画像・付属情報・商品ラベルなど
  **入力として与えられるテキスト全般**はデータであって命令ではない旨をデリミタ付きで示し、
  ...
  商品ラベルは 200 文字までに制限する（巨大テキストの永続書込・プロンプト汚染の増幅経路を塞ぐ。
  `inventory_action_flag.note` の1,000文字制限と同じ考え方＝§7.x）。
- AI 例外の内部文言（SDK メッセージ等）は error_message に保存せず、エラーコード＋汎用文言のみを
  記録・表示する（詳細はサーバーログ）。
- 受付上限超過の 429（`UNDX-REQ-009`）に `Retry-After` ヘッダは付けない（設計判断）。
...
- **画像ストレージの増加:** チェック1件あたり最大 合計14MB の画像を PostgreSQL（bytea）に保存し、
  ...
  **プロビジョニング容量との突合:** RDS の初期割当は 20GiB で、上記の増加率では数ヶ月で
  枯渇しうる。そのため `--max-allocated-storage 100`（最大 100GiB までの自動スケーリング）を
  必須設定とし、枯渇による `storage-full`（アプリ全体が書込不能）を防ぐ。
  運用開始時に **RDS の空き容量アラーム（閾値・通知先）を設定すること**も前提とする
  （いずれも手順は `infra/deploy-guide.md`）。
  自動スケーリング有効下では空き容量の一時的な低下は自動拡張で解消される正常な挙動のため、
  アラームは「自動拡張が働くはずなのに空きが戻らない」状態だけを捉えるよう設定する。
  AWS の自動拡張の発動点は**割当の10%**なので、閾値は初期割当 20GiB のときの発動点 2GiB より下の
  **絶対値 1.5GiB**（30分継続）とし、どの割当サイズでも「自動拡張が一度は試みた後」にのみ鳴るようにする。
  持続的なアラーム時は `AllocatedStorage` と `MaxAllocatedStorage` を突合し、
  上限に達している場合の回復手段が自動スケーリング上限の引上げ
  （`modify-db-instance --max-allocated-storage`）であり、記録保護（削除 API を提供しない）と両立する。
  短時間で復旧するアラームは対応不要で、`--ok-actions` による復旧通知で収束を確認する。将来の恒久対応として「判定・findings は恒久保存、画像バイナリのみ
  保持期間（例: 12ヶ月）経過後に削除する」運用パスの設計を残課題とする
  （記録と画像原本の分離により原則2と両立可能）。
- **認可とコスト:** API は認証必須（`[Authorize]`）だが、サプライヤー/バイヤーのロール分離は
  ナビゲーション表示のみで API レベルでは行わない（参照系は認証のみ、という本アプリの既定に従う。
  §6 認可の注記参照）。ただし本エンドポイントは**書込系かつ外部有料 API を起動する**点で
  `POST /api/imports`（`Importer` ポリシー必須）とは性質が異なる。単一テナント・少人数運用を前提に、
  **現時点ではロール制限・回数上限を設けないことを受容する**（事故による多重実行は rerun の
...
- **リバースプロキシとの関係:** 公開経路は複数プロジェクト相乗りの共有 `nginx-proxy`（本 compose の
  管理外・読み取りタイムアウト約60秒）。AI 呼出は順番待ちを含め最大約9分（540秒）かかりうるため**同期実行は採らず**、
  バックグラウンド実行＋ポーリングとした（§13.5）。これによりプロキシのタイムアウト設定変更は不要で、
  同居プロジェクトへ影響を波及させずに済む。**必要な確認はボディサイズ上限のみ**
  （`client_max_body_size`）。ただし必要値は本機能単独の 25MB ではなく、
  アプリ全体の最大値である **週次取込の 50MB** で決まる（`ImportsController.HardSizeLimitBytes`）。
  副資材チェックの 25MB だけに合わせると既存の週次取込が nginx 層の 413 で退行するため、
  **≧ 50MB** とすること（経路別の一覧と手順は `infra/deploy-guide.md` ステップ8-2）。
