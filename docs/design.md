...
- **AI 呼出は同時実行 1 件に直列化**（SemaphoreSlim）し、順番待ちは 420 秒（7分）・AI 呼出自体は 120 秒で
  ...
  **収支外の経路:** 次の2つは AI 実行の受付枠・セマフォの管理外であり、上記の約310MiB には含まれない。
  - **画像配信**（`GET /api/subsidiary-check/{id}/images/{imageId}`）: 詳細画面は最大13枚を取得し、
    ...
    （`SubsidiaryCheckService.MaxConcurrentImageDownloads`）。1枚あたりのピークは byte[]（最大5MiB）＋
    応答バッファ ≒ 約10MiB で、4 並列＝**約40MiB がこの経路の上限**。**同時閲覧者が何人いても
    この値を超えない**（閲覧者数に比例しない）ことが要点。読み出しはセマフォ取得後に行うため、
    順番待ち中のリクエストは画像バイト列を保持せず、待機件数はピークに寄与しない。
  ...
  - **DB 読取の一時コピー**: Npgsql は 8KB を超える行にオーバーサイズバッファを確保し、接続がプールへ
    ...
    byte[] とは別に同量程度の一時割当が乗るため、rerun の画像読取は実効で1件あたり約2倍を見込む。
...
- **画像ストレージの増加:** チェック1件あたり最大 合計20MB の画像を PostgreSQL（bytea）に保存し、
  ...
  **プロビジョニング容量との突合:** RDS の初期割当は 20GiB で、上記の増加率では数ヶ月で
  ...
  アラームは**絶対値 5GiB が 30分継続した場合**（＝自動拡張が上限に追随できていない状態）に
  意味を持つよう設定する。この持続的なアラームに対する回復手段が自動スケーリング上限の引上げ
