...
- **AI 呼出は同時実行 1 件に直列化**（SemaphoreSlim）し、順番待ちは 420 秒（7分）・AI 呼出自体は 120 秒で
  ...
  **収支外の経路（受容した既知のリスク）:** 次の2つは受付枠・セマフォの管理外であり、
  上記の約310MiB には含まれていない。同時実行が重なるとヘッドルーム（約74MiB）を侵食しうる。
  - **画像配信**（`GET /api/subsidiary-check/{id}/images/{imageId}`）: 詳細画面は最大13枚を並列取得し、
    サーバー側は1枚ずつ byte[] を実体化して返す。1チェック分は合計20MiB 以内だが、**同時閲覧者数に比例**する。
  ...
  **定量的な境界:** 画像配信は1閲覧あたり byte[] 20MiB ＋ Npgsql の一時割当 同量程度 ≒ **約40MiB** を要するため、
  **詳細画面を同時に2名が開くとヘッドルーム約74MiB を超えうる**（40MiB × 2 = 80MiB）。
  現状の利用規模（日次数件〜10件・少人数）ではこの同時性は稀として受容するが、
  **成立条件は「api コンテナのメモリ監視が機能していること」**である（手順は `infra/deploy-guide.md`
  の**ステップ8-3**。ホスト全体のメモリ指標ではコンテナ OOM を検知できないため、
...
- **画像ストレージの増加:** チェック1件あたり最大 合計20MB の画像を PostgreSQL（bytea）に保存し、
  ...
  **プロビジョニング容量との突合:** RDS の初期割当は 20GiB で、上記の増加率では数ヶ月で
  ...
  運用開始時に **RDS のストレージ使用率アラーム（閾値・通知先）を設定すること**も前提とする
  （いずれも手順は `infra/deploy-guide.md`）。アラーム到達時の回復手段は
  自動スケーリングの上限引上げ（`modify-db-instance --max-allocated-storage`）であり、
  記録保護（削除 API を提供しない）と両立する。将来の恒久対応として「判定・findings は恒久保存、画像バイナリのみ
