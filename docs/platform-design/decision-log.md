...
**決定**
入力は画像（指示書1〜3枚・タグ1〜10枚）とし、比較判定は Anthropic Claude の画像認識で行う（チェック3要素: レイアウト／順番／内容）。規定ルール（アイコン優先順位・禁止用語・表示順序・寸法）は Core `SubsidiaryCheckRuleCatalog` を単一 SoT とし、AI プロンプトのルールテキストとルールブック API 応答を同一カタログから生成する。チェック記録は `subsidiary_check` に不変記録として保存し、再実行は failed および孤児化した processing（最後の実行開始 `started_at` から10分超）のみ許可し、入口で条件付き UPDATE による原子的クレームを行って重複実行を構造的に排除する（completed は API・DB 層の両方で上書き保護＝記録保護）。付属情報は `m_product_attachment`（additive・nullable）で受け、登録済みの場合のみ AI プロンプトに注入する。AI 実行は**バックグラウンドで非同期実行**し、POST は記録を作成して即応答する（フロントは状態をポーリング。共有 `nginx-proxy` の約60秒タイムアウトを超える処理を同期で行わないため、mart 再構築で確立済みの方式を踏襲＝ADR-009 と同じ構造）。同時1件へ直列化し、順番待ち30秒・AI 呼出120秒のタイムアウトと実行総数の上限（超過は429）で保護する。画像は各5MB・合計20MB 以内（Anthropic API のリクエスト上限 32MB への安全側設計）。
