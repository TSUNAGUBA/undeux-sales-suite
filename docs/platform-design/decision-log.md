...
**決定**
入力は画像（指示書1〜3枚・タグ1〜10枚）とし、比較判定は Anthropic Claude の画像認識で行う（チェック3要素: レイアウト／順番／内容）。規定ルール（アイコン優先順位・禁止用語・表示順序・寸法）は Core `SubsidiaryCheckRuleCatalog` を単一 SoT とし、AI プロンプトのルールテキストとルールブック API 応答を同一カタログから生成する。チェック記録は `subsidiary_check` に不変記録として保存し、再実行は failed および孤児化した processing（最後の実行開始 `started_at` から20分超）のみ許可し、入口で条件付き UPDATE による原子的クレームを行って重複実行を構造的に排除する（completed は API・DB 層の両方で上書き保護＝記録保護）。付属情報は `m_product_attachment`（additive・nullable）で受け、登録済みの場合のみ AI プロンプトに注入する。AI 実行は**バックグラウンドで非同期実行**し、POST は記録を作成して即応答する（フロントは状態をポーリング。共有 `nginx-proxy` の約60秒タイムアウトを超える処理を同期で行わないため、mart 再構築で確立済みの方式を踏襲＝ADR-009 と同じ構造）。同時1件へ直列化し、順番待ち420秒・AI 呼出120秒のタイムアウトと実行総数の上限4件（超過は429）で保護する。画像は各5MB・合計20MB 以内（Anthropic API のリクエスト上限 32MB への安全側設計）。

**代替案**
1. プロトタイプのルールエンジン＋フォーム手入力の移植（AI 画像比較なし）
2. 画像から構造化抽出（アイコン・文言）→決定的ルールエンジン適用の二段構成
3. 同期実行のまま共有プロキシのタイムアウトを延長する（当初案。共有 EC2 の他プロジェクトへ波及するため不採用）

**根拠**
実業務の入力は画像であり、タグ内容の手入力はチェック1件ごとの負荷が高く誤転記リスクもある（案1は業務フロー不整合）。案2は再現性で優るが抽出スキーマの安定化が前提で初期リリースの範囲を超える（将来の発展形として有望）。案3は共有 EC2 に相乗りする他プロジェクトの全 vhost へ影響が波及するため採れず、代わりに mart 再構築（ADR-009）で確立済みの「バックグラウンド実行＋状態ポーリング」を踏襲した（原則3。新規のジョブ基盤を作らずに済む）。AI 直接比較＋確証が持てない項目は warn（目視誘導）に倒す設計が、最小構成で「目視チェックの補助」という業務価値を出す。

**トレードオフ**
AI 判定は非決定的で、同一入力に対する再現性は決定的ルールエンジンに劣る（warn 正規化と「AI は補助・最終判断は人」の位置づけで緩和）。非同期実行のため結果は即座には返らず、フロントにポーリングと processing 表示が必要になる。バックグラウンドタスクはプロセス停止で失われ processing が孤児化しうるが、`started_at` の滞留判定＋rerun で回復できる（永続キューは持たない）。findings の jsonb 保存は指摘単位の横断分析に不向き（要件が生じた時点で正規化を検討）。api コンテナのメモリ上限（512MiB）に対し1リクエストの保持量が大きいため AI 呼出を同時1件に直列化しており、利用者増時はスループットが制約になる（対処はメモリ上限の引上げが先。`design.md` §13.5）。
...
- エラーコード: `UNDX-REQ-004`〜`007`（画像検証）、`UNDX-REQ-008`（アップロード合計サイズ超過・transport 上限の共通マップ）、`UNDX-REQ-009`（受付上限超過・429）、`UNDX-DATA-005`（チェック未存在）、`UNDX-AI-009`（応答解析失敗）
