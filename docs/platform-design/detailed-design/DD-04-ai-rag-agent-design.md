...
| `UNDX-AI-009` | AI 応答の解析失敗（構造化出力の JSON が抽出不能） | 非ブロッキング（実行記録を failed で保全し再実行を許可。**現行アプリの副資材チェックで採番済み** — `design.md` §13・ADR-018） |

> `UNDX-AI-008`（AI 未設定・503）は現行アプリで採番済み（`design.md` §8）。002〜007 は本書の予約を維持し、現行アプリの新規採番は 009 以降を用いる。
