...
- **非同期実行:** 約160万行規模の集約は共有 `nginx-proxy` のタイムアウト（約60秒）を超えるため、`POST /api/mart/rebuild` は実行権を取得して即応答し、本体はバックグラウンドで実行。状態は `build_info.status`（idle/running/completed/failed）で管理し、フロントは `GET /api/mart/status` をポーリングする。コマンドタイムアウトは 30 分、それより大きい 45 分以上滞留した `running` は stale とみなし再実行を許可する（正常実行を stale 誤認しない）。
