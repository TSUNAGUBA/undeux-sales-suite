...
const props = defineProps<{
  ...
  clickable?: boolean
  /**
   * 残在庫数（倉庫在庫・モック）と残在庫金額（原価ベース・最新週スナップショット）列を
   * 末尾（変動列の右）に追加するか。商品記号別ランキングで倉庫在庫の積み上がりを併記するために使う。
   * 残在庫数は倉庫在庫のモック値（常時表示の「店頭在庫数」とは別物）、その右に残在庫金額を並べる。
   */
...
/** 残在庫数（倉庫在庫・モック。未取得は '—'）。 */
function warehouseStockText(row: RankingViewRow): string {
  return row.warehouseStock === null ? '—' : formatNumber(row.warehouseStock)
}
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          >
            ...
            <template v-if="showComparison">
              ...
              <template v-if="showStockColumns">
                <td class="whitespace-nowrap px-3 py-2 text-right tabular-nums text-slate-600">{{ warehouseStockText(row) }}</td>
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-x-3 gap-y-1 text-sm">
          ...
          <div v-if="showStockColumns" class="flex justify-between gap-2">
            ...
            <span class="tabular-nums text-slate-700">{{ warehouseStockText(row) }}</span>
          </div>
          <div v-if="showStockColumns" class="flex justify-between gap-2">
