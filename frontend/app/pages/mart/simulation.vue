 ...
 * （mart は週次系列の素材のみ返す）。気温は mart の気温データ（実測 dim_climate、未カバー週は
 * 標準気候へフォールバック）。
 */
...
<template>
  <div class="space-y-4">
    ...
    <StatusBlock
      :loading="loading"
      :error="errorMessage"
      :empty="isBuilt && (weekly?.points.length ?? 0) === 0"
      empty-message="該当するデータがありません。フィルタや期間を見直してください。"
    >
      <MartNotBuiltNotice v-if="!isBuilt" />
      <div v-else-if="!regression" class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-700">
