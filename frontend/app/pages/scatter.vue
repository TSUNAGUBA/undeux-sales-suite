...
/**
 * 散布図・回帰分析（/scatter）ページ。
 *
 * モードA【MD・発注向け】「週平均(最高/最低)気温 × 週売上数量」（点=各週）
 *   単回帰直線で「スイッチ温度（商品が動き出す境界線）」を可視化する。
 * モードB【在庫・販促向け】「消化率 × 値引き率」（点=型番、バブル=売上数量）
 *   4象限（お宝/危険/大爆死/好調）で値下げ判断を支援する。
 *
 * データフロー（SoT）: バックエンドは集計素材（週次系列・型番別指標）のみ返し、
 * 回帰直線・象限分類はフロント（utils/regression）で算出する表示射影。
 */
...
async function load(): Promise<void> {
  ...
  try {
    if (mode.value === 'temperature') {
      ...
      markdown.value = await get<MarkdownScatterResponse>('/api/analysis/markdown', toQuery())
    }
  } catch (error) {
  ...
  <div class="space-y-4">
    <div>
      ...
      <p class="text-sm text-slate-500">
        気温×売上で「スイッチ温度」を、消化率×値引き率で値下げ判断（4象限）を可視化します。
      </p>
    </div>
    ...
    >
      <!-- モードA -->
      <div v-if="mode === 'temperature'" class="space-y-4">
