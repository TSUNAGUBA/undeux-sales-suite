...
#
# GitHub Actions（.github/workflows/deploy.yml）の各テストゲート（単体テスト・
# 結合テスト・シナリオテスト）から呼び出される共通実行ラッパー。
# 副資材チェック機能の原型リポジトリ（subsidiary-materials-agent）の同名スクリプトと
# 同一の流儀に揃える（原則3）。
#
#   使い方:
#     STAGE_CMD="dotnet test ..." bash scripts/run-test-stage.sh "単体テスト" unit-test
#
#   引数:
#     $1 = ステージ表示名（例: 単体テスト）
#     $2 = ログファイル名（拡張子なし。例: unit-test）
#   環境変数:
#     STAGE_CMD          = 実行するコマンド（ワークフローの各ステップで注入する）
#     ARTIFACT_NAME      = ログのアップロード先アーティファクト名（既定: deploy-logs）
#     SCRUB_SECRET_NAMES = ログ・サマリー保存前に値を除去する環境変数名の
#                          スペース区切りリスト（未設定なら除去なし）
#
...
if [ -z "${STAGE_CMD:-}" ]; then
  echo "::error::${STAGE_NAME} の実行コマンド（STAGE_CMD）が設定されていません。ワークフロー定義を確認してください。"
  exit 1
fi
...
scrub_secrets() {
  ...
  for name in ${SCRUB_SECRET_NAMES:-}; do
    value="${!name-}"
    if [ -n "$value" ]; then
      text="${text//"$value"/***}"
    fi
